   ...
   implements IPlugin
 {
   ...
   public boolean start()
   {
     ...
     try
     {
       ...
       job.getJobDataMap().put("webSite", JFinal.me().getServletContext().getAttribute("webSite"));
 
       siteMapJob.getJobDataMap().put("webHome", "http://blog.94fzb.com:8080");
       siteMapJob.getJobDataMap().put("webDiskPath", JFinal.me().getServletContext().getRealPath("sitemap.xml"));
       CronTrigger trigger = 
         (CronTrigger)TriggerBuilder.newTrigger()
         .withIdentity("siteMap", "util")
         .withSchedule(CronScheduleBuilder.cronSchedule("0 15 23 * * ? *"))
         .build();
